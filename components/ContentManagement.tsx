    ...
    <div className="container mx-auto py-12">
      ...
      {canPerformActions() ? (
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-between gap-2 w-full">
            <span className="text-xs opacity-50">
              Author:{" "}
              {String(clerkUser?.firstName) +
                " " +
                String(clerkUser?.lastName) || "Anonymous"}
            </span>
            <span className="text-xs opacity-50">
        ...
        <p className="text-red-500">
          You do not have permission to create, edit, or publish blogs. Only
          Authors or Site_Admin users can perform these actions.
        </p>
      )}
    </div>
  );
